...
in stdenv.mkDerivation (finalAttrs: {
  url = "https://developer.modular.com/magic";
  # pname = "magic";
  # version = "6.3.3";
  ...
  fetchedContent = fetchurl {
    url = url;
  };
  
  # Save the content to a temporary file
  tempFile = builtins.toFile "magic-script" fetchedContent;
  
  # Execute the script and capture its output
  result = runCommand "execute-magic-script" {} ''
    ${pkgs.coreutils}/bin/chmod +x ${tempFile}
    OUTPUT=$(${pkgs.bash}/bin/bash ${tempFile})
    rm -f ${tempFile}
    echo "$OUTPUT"
  '';
